...
out_dir = Path(__file__).parent.parent / "output" / \
        experiment_name / model_name / "render_data"
out_dir = str(out_dir)
interval = cfg['interval']
frame_rate = round(1 / interval)


# -------- inputs --------
fluid_files = series(f"{out_dir}/fluid_frame_*.vtk")  # XML ImageData, mislabeled .vtk
solid_files = series(f"{out_dir}/solid_frame_*.vtk")  # XML UnstructuredGrid, mislabeled .vtk
...
cont.Isosurfaces = [0.5, 1.0, 2.5, 5.0, 10.0]
...
# -------- save AVI --------
SaveAnimation(f"{out_dir}/video_{view_mode}.avi", view, FrameRate=frame_rate, ImageResolution=[1920, 1080])
